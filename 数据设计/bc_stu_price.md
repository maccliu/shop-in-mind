...
CREATE TABLE `bc_stu_price` (
  ...
  UNIQUE KEY `uni_key` (`stu_id`,`user_rank`,`price_type`,`currency`,`price`,`start_at`,`end_at`,`free_shipping`),
  KEY `stu_id` (`stu_id`),
...
> 备注:
> 1. 价格根据 `price_type` 进行了分层，高层价格会覆盖底层价格。满足(stu_id, user_rank, time) 条件的最高`price_type`的价格即为实际价格。
